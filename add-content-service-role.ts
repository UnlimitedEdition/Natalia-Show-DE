// Ova skripta koristi service role za zaobilaženje RLS politika
import { createClient } from '@supabase/supabase-js'

// Dobijamo service role ključ iz environment varijabli
const supabaseService = createClient(
  import.meta.env.VITE_SUPABASE_URL,
  import.meta.env.VITE_SUPABASE_SERVICE_ROLE_KEY // Service role ključ zaobilazi RLS politike
)

const addContentWithServiceRole = async () => {
  console.log('Dodajem sadržaj koristeći service role...')

  // Placeholder tekstualni sadržaj za sve sekcije na svim jezicima
  const contentData = [
    // Hero sekcija
    {
      section_key: 'hero',
  ...
  try {
    // Ubacujemo tekstualni sadržaj koristeći service role
    console.log('Dodajem tekstualni sadržaj...')
    const { error: contentError } = await supabaseService
      .from('content')
    ...
    console.error('Greška pri dodavanju placeholder sadržaja:', error)
  }
}

addContentWithServiceRole()