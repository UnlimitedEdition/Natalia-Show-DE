...
const addPlaceholderContent = async () => {
  console.log('Dodajem placeholder sadržaj u bazu podataka...')

  // Proveravamo da li smo prijavljeni
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) {
    console.log('Niste prijavljeni. Molimo prvo se prijavite kao admin korisnik.')
    return
  }

  console.log('Prijavljeni ste kao:', user.email)

  // Dodajemo placeholder tekstualni sadržaj za sve sekcije na svim jezicima
  const contentData = [
    // Hero sekcija
    {
      section_key: 'hero',
