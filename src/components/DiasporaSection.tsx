...
const DiasporaSection = () => {
  ...
  const [diasporaMedia, setDiasporaMedia] = useState<ExtendedMedia[]>([])
  const [backgroundImage, setBackgroundImage] = useState('')
  const [backgroundVideo, setBackgroundVideo] = useState('')
  const [title, setTitle] = useState('Our Global Family')
  const [subtitle, setSubtitle] = useState('Diaspora Community')
  const [description, setDescription] = useState('Connecting with our diaspora around the world')

  const fetchDiasporaContent = useCallback(async () => {
    try {
      const { data, error } = await supabase
        ...
        `)
        .eq('section_key', 'diaspora')
        .single()
      
      if (error) {
        console.error('Error fetching diaspora content:', error)
        return
      }
      ...
      if (contentData) {
        ...
        setTitle(currentLangContent['title'] || 'Our Global Family');
        setSubtitle(currentLangContent['subtitle'] || 'Diaspora Community');
        setDescription(currentLangContent['description'] || 'Connecting with our diaspora around the world');
      }
    } catch (error) {
      console.error('Error fetching diaspora content:', error)
    }
  ...
  const fetchDiasporaMedia = useCallback(async () => {
    try {
      ...
      if (mediaError) {
        console.error('Error fetching diaspora media:', mediaError)
        return
      }
      ...
      if (mediaData) {
        ...
        if (contentError) {
          console.error('Error fetching diaspora media content:', contentError)
          return
        }
        ...
        setDiasporaMedia(combinedData);
      }
    } catch (error) {
      console.error('Error fetching diaspora media:', error)
    }
  }, [])
  ...
  useEffect(() => {
    ...
    fetchDiasporaMedia()
  }, [fetchDiasporaContent, fetchDiasporaMedia, currentLanguage])
            ...
            >
              {media.file_url && (
                <div className="relative pb-[56.25%]"> {/* 16:9 aspect ratio */}
                  <img 
                    src={media.file_url} 
                    alt={media.title || 'Diaspora Event'} 
                    className="absolute inset-0 w-full h-full object-cover"
                  />
